...
function MoneyFactory() {
	...
	this.createMoney = function(monster, money, res) {
		var data = {};
			...
			data.name = "金币";
			data.money = money;
			data.animation = new SpriteAnimation(res, 300, 0);
...
function ThingsFactory() {
	this.createThing = function(monster, type, amount, res, name) {
		var data = {};
			...
			data.amount = amount;
			data.name = name;
			data.animation = new SpriteAnimation(res, 1000, 0);
...
function LevelUpEffect(player) {
	this.animation = new SpriteAnimation(window.resource.effect["LevelUp2"], 2000, 0);
	this.curr_res;
