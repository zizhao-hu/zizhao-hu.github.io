...
// Fixed virtual resolution. CSS transform: scale(N) on the stage makes the canvas +
// any HTML overlays (e.g. TV iframe) grow together. Use "cover" (max) so the game
// fills the entire viewport with no black margins — content outside the aspect
// ratio is clipped by body's overflow: hidden.
window.GAME_W = 1280;
window.GAME_H = 720;
window.fitCanvas = function() {
	var c = $("canvas");
	if (!c) return;
	if (c.width !== window.GAME_W) c.width = window.GAME_W;
	if (c.height !== window.GAME_H) c.height = window.GAME_H;
	window.WIDTH = window.GAME_W;
	window.HEIGHT = window.GAME_H;
	var s = Math.max(window.innerWidth / window.GAME_W, window.innerHeight / window.GAME_H);
	var stage = $("stage");
	if (stage) stage.style.transform = 'scale(' + s + ')';
};

window.onload = function() {
	// Hide the home/start UI completely — we drop straight into the game.
	var home = $("home");
	...
	this.thread = setInterval(function() {
		if (window.resource.curr_amount == window.resource.total_amount) {
			...
			// Skip the home screen and jump straight into the game with a male character.
			start("male");
		} else {
			ctx.clearRect(0, 0, window.WIDTH, window.HEIGHT);
			ctx.fillText("Loading game resources, please wait...  " + (window.resource.curr_amount / window.resource.total_amount * 100).toFixed(2) + "%", window.WIDTH / 2,  window.HEIGHT / 2);
		}
...
window.start = function(gender) {
	// Remove home overlay and audio if they exist (they may live inside .stage now).
	var home = $("home");
	if (home && home.parentNode) home.parentNode.removeChild(home);
	var homeAudio = $("home_audio");
	if (homeAudio && homeAudio.parentNode) homeAudio.parentNode.removeChild(homeAudio);
	...
	// Start at Ludibrium town (custom home map)
	var game_scene = new GameScene(getGameData({position: "ludibrium", orientation: window.EAST}), ctx);
	...
	function getGameData(next_map) {
		...
		var bg_data = {x: 0, y: 0, res: window.resource.bg["bg"][map_basic_data.bg],
			useLudibriumSky: next_map.position === 'ludibrium'};
		...
		var scene_obj = {};
			...
			scene_obj.map = new GameMap(map_data);
			...
			scene_obj.npcs = map_basic_data.npcs || [];
	...
	function initSingle() {
		...
		var player_data = {x: 0, y: 0, width: window.resource.player["stand1"][0].width, height: window.resource.player["stand1"][0].height, 
						   ...
						   stand_animation: new SpriteAnimation(window.resource.player["stand1"], 800),
						   walk_animation: new SpriteAnimation(window.resource.player["walk1"], 800),
						   jump_animation: new SpriteAnimation(window.resource.player["jump"], 800),
						   rope_animation: new SpriteAnimation(window.resource.player["rope"], 500),
						   ladder_animation: new SpriteAnimation(window.resource.player["ladder"], 500)};
