...
function MonsterSkillEffectFactory() {
	this.createSkillEffect = function(monster, player_center_x, player_center_y, speed, res) {
		var data = {};
			...
			data.animation = new SpriteAnimation(res, 500);
...
function MonsterSkillHitFactory() {
	this.createSkillHit = function(x, y, time, res) {
		var data = {};
			data.animation = new SpriteAnimation(res, time);
...
function Rect(x, y, width, height) {
	...
	this.collision = function(rect) {
		// True AABB overlap. The original code only counted a hit if a CORNER
		// of one rect was inside the other, which silently misses the case
		// where one rect is wider but shorter (or vice versa) — e.g. the
		// player's 17px-wide map_rect crossing through a 16px-wide ladder
		// rect. Both rects overlap but no corner is contained, so the old
		// check returned false and ladders couldn't be grabbed.
...
function SpriteAnimation(res, time, flag_frame) {
	this.res = res;
	this.count = 0;
