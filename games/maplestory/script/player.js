function Player(data) {
	...
	this.rope = function(item_center_x, is_ladder) {
		// Grab when pressing UP from any non-rope state (stand / walk / fall),
		// or DOWN from a stationary state. The original code only grabbed
		// from is_fall, which forced the player to jump-and-hit-up to start
		// climbing — unintuitive and the reason "ladders don't work" felt true.
		var grab_up = this.is_up && (this.is_fall || this.is_stand || this.is_walk);
		var grab_down = this.is_down && (this.is_stand || this.is_walk);
		...
		// Original code unconditionally fell after a missed grab — that's
		// only correct mid-air. If we're standing on a foothold and just
		// happen to overlap a ladder rect, falling would punt the player
		// off the floor every frame.
		if (!this.is_rope && this.is_fall) {
			this.fall();
		}
	}
	...
	this.draw = function(ctx) {
		...
		// Bottom-center anchor: each animation frame can have a different bounding
		// box (jump may include cape/wisp decor on one side, swing extends an arc),
		// so we center the frame horizontally on the player's center and bottom-
		// align it to the player's feet. Without this the body visually displaces
		// every time the action changes.
		var img = this.curr_res;
		var dx = (this.x + this.width / 2 - img.width / 2) + this.fix_x;
		var dy = (this.y + this.height - img.height) + this.fix_y + window.PLAYER_OFFSET_Y;
		if (this.is_right) {
			ctx.drawRightImage(img, dx, dy);
		} else {
